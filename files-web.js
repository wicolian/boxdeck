...
const md2html = md => marked ? marked.parse(md) : `<pre>${esc(md)}</pre>`;
...
http.createServer((req, res) => {
  ...
  let st; try { st = fs.statSync(abs); } catch { res.writeHead(404); return res.end('not found: ' + url); }
  if (st.isDirectory()) {
  ...
  if (ext === '.md' || ext === '.markdown') { res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
  ...
  res.writeHead(200, { 'Content-Type': MIME[ext] || 'application/octet-stream', 'Content-Length': st.size }); fs.createReadStream(abs).pipe(res);
